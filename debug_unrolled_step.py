...
def main():
	...
	w_var=utils.get_var(tf.compat.v1.trainable_variables(), 'weight_var')[1]
	...
	unrolled_w_var=utils.get_var(tf.compat.v1.trainable_variables(), 'unrolled_weight_var')[1]
	cpoy_weight_opts=[v_.assign(v) for v_,v in zip(unrolled_w_var,w_var)]

	with tf.control_dependencies(cpoy_weight_opts):
		unrolled_optimizer=tf.compat.v1.train.GradientDescentOptimizer(0.001)
	...
	with tf.control_dependencies([unrolled_optimizer]):
		valid_grads=tf.gradients(ys=unrolled_train_loss,xs=unrolled_w_var)

	R=0.01/tf.linalg.global_norm(valid_grads)
	...
	optimizer1=tf.compat.v1.train.GradientDescentOptimizer(R)
	optimizer1=optimizer1.apply_gradients(zip(valid_grads,w_var))
	with tf.control_dependencies([optimizer1]):
		arch_grad_after=tf.gradients(ys=train_loss,xs=arch_var)
